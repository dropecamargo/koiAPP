...
class Prodbodelote extends Model
{
	...
	*/
    ...
    public  static function actualizar(Producto $producto, $sucursal, $tipo, $unidades, $lote)
    {   
        // Validar sucursal
		...
		$prodbodelote = Prodbodelote::where('prodbodelote_serie',$producto->id)->where('prodbodelote_sucursal',$sucursal->id)->where('prodbodelote_lote', $lote)->first();
        if(!$prodbodelote instanceof Prodbodelote){
        	$prodbodelote = new Prodbodelote;
	        $prodbodelote->prodbodelote_lote = $lote;	
