...
class Pedidoc2 extends Model
{
	...
	*/
	...
	public static function getPedidoc2($id)
	{
		...
		$query->select('pedidoc2.*','producto_serie','producto_nombre','producto_maneja_serie', 'producto_unidad',DB::raw("((((pedidoc2_precio_venta - pedidoc2_descuento_valor) * pedidoc2_cantidad) * 100) / (pedidoc2_costo * pedidoc2_cantidad)) AS pedidoc2_margen_porcentaje"), 'pedidoc1_autorizacion_co');

        $query->join('producto', 'pedidoc2_producto', '=' ,'producto.id');
        $query->join('pedidoc1', 'pedidoc2_pedidoc1', '=' ,'pedidoc1.id');
