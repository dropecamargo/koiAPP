...
use App\Models\Contabilidad\PlanCuentaNif, App\Models\Contabilidad\Documento, App\Models\Base\Tercero, App\Models\Contabilidad\AsientoNif, App\Models\Contabilidad\AsientoNif2, App\Models\Contabilidad\CentroCosto, App\Models\Contabilidad\SaldoContableNif, App\Models\Contabilidad\SaldoTerceroNif, App\Models\Base\Empresa;
	...
	public function saldosTerceros(PlanCuentaNif $cuenta, Tercero $tercero, $naturaleza, $debito = 0, $credito = 0, $xmes, $xano)
	{
    	...
    	while (true)
		{
			...
			$sql = "
				...
				FROM saldostercerosn as s1, plancuentasn
			...
			$sql .= "
				and s1.saldostercerosn_cuenta = plancuentasn.id
	...
	public function saldosContables(PlanCuentaNif $cuenta, $naturaleza, $debito = 0, $credito = 0, $xmes, $xano)
	{
        ...
        foreach ($cuentas as $item) {
        	...
        	while (true)
			{
	    		...
	    		$sql = "
					SELECT plancuentasn.id as plancuentasn_id, plancuentasn_nombre, plancuentasn_cuenta, plancuentasn_naturaleza,
					(select (CASE when plancuentasn_naturaleza = 'D'
							...
							END)
						FROM saldoscontablesn
						WHERE saldoscontablesn_mes = $xmes2
						and saldoscontablesn_ano = $xano2
						and saldoscontablesn_cuenta = plancuentasn.id
					) as inicial,
					...
					FROM plancuentasn
					WHERE plancuentasn.id IN (
						SELECT s.saldoscontablesn_cuenta
						FROM saldoscontablesn as s
					...
					and plancuentasn.id = {$objCuenta->id} order by plancuentasn_cuenta ASC";
		        $arSaldos = DB::select($sql);
		        if(!is_array($arSaldos)) {
