...
class CreateSaldostercerosnTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('saldostercerosn', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            ...
            $table->foreign('saldostercerosn_cuenta')->references('id')->on('plancuentasn')->onDelete('restrict');
            $table->foreign('saldostercerosn_tercero')->references('id')->on('tercero')->onDelete('restrict');
    ...
    {
        Schema::dropIfExists('saldostercerosn');
    }
}
