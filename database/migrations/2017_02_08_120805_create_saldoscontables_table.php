...
class CreateSaldoscontablesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('saldoscontables', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            ...
            $table->foreign('saldoscontables_cuenta')->references('id')->on('plancuentas')->onDelete('restrict');
            $table->unique(['saldoscontables_cuenta', 'saldoscontables_ano', 'saldoscontables_mes'], 'saldoscontables_cuenta_ano_mes_unique');
        });
    }
    ...
    {
        Schema::dropIfExists('saldoscontables');
    }
}
